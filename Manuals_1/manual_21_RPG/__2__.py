# ДЗ
# Добавь в игру Финального босса.
# Пусть по достижению 20-го уровня игроку попадётся сильный босс «Кентавр».
# Его будет непросто победить и от которого невозможно сбежать.
...
class Enemy:
    ...
    def create_centaur(self):
        self.name = choice(list(self.centaur.keys()))
        self.hp = self.centaur[self.name][0]
        self.damage = self.centaur[self.name][1]
        self.xp = self.hp * 2.5
...
def start(heal=None):
    if heal is None:
        enemy = Enemy()
        if hero.lvl in [20, 21]:
            enemy.create_centaur()
    else:
        enemy = heal
